...
export default async function TraceabilityPage({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  const recallLotId = positiveInteger(value(params.recall_lote));
  ...
  return <main className="app-shell">
    <section className="workspace technical-workspace traceability-workspace">
      ...
      <details className="panel form-panel" open={Boolean(recallLotId)}><summary><strong>Simular recolhimento</strong><span>não bloqueia nem movimenta lotes</span></summary><form className="form-grid" method="get"><input type="hidden" name="simular" value="1" /><label>Tipo do lote<select name="recall_tipo" defaultValue={recallType}><option value="MP">Matéria-prima</option><option value="EMBALAGEM">Embalagem</option><option value="PI">Produto intermediário</option><option value="PA">Produto acabado</option></select></label><label>Código interno do lote<input name="recall_lote" inputMode="numeric" defaultValue={recallLotId ?? ""} required /></label><div className="form-footer wide-field"><span>A simulação mostra somente destinos com expedição líquida ativa.</span><button className="secondary-button" type="submit">Simular impacto</button></div></form></details>
