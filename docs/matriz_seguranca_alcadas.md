...
|---|---|---|---|---|---|
| seguranca | usuario autenticado ve proprio perfil; administrador ve email e confirmacao do diretorio Auth | usuarios/permissoes por RPC; convite Auth por boundary server-only; troca do proprio email exige confirmacao | sim | `system.admin`, `security.manage_users`, `security.manage_permissions`, `security.change_own_password`, `security.change_own_email` | escrita direta restringida na 0035; 0038 permanece para conta legada; 0046 adiciona recuperacao; 0047 substitui novo acesso por convite verificavel, bloqueia ativacao pendente e audita troca de email por hash |
