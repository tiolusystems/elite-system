-- F1.1: make product creation and its initial shelf life one atomic audited
-- database operation. Existing positional callers remain compatible because
-- the new parameter is optional and appended to the original signature.

drop function if exists public.create_cad_produto_base(
  text, text, text, text, text, numeric, text, text, text, text, jsonb
);
...
revoke all on function public.create_cad_produto_base(
  text, text, text, text, text, numeric, text, text, text, text, jsonb, integer
) from public, anon;
grant execute on function public.create_cad_produto_base(
  text, text, text, text, text, numeric, text, text, text, text, jsonb, integer
) to authenticated;
...
comment on function public.create_cad_produto_base(
  ...
  'Creates a product and its initial shelf life atomically through the audited cadastros boundary.';
