...
| `DEC-007` | Catalogos tecnicos e especificacoes | normalizar nutrientes por FK, governar unidades/aliases e criar especificacoes versionadas de produto para pH e demais faixas; separar garantia calculada da garantia documental MAPA | cadastros tecnicos, formulas, garantias e CQ | pendente |
| `DEC-008` | Embalagem, composicao e logistica | criar BOM versionada de embalagem, peso, cubagem e vinculo de entregador/veiculo ao romaneio; transformacao historica somente quando comprovada ou marcada como inferida | embalagem, custo, estoque PA/PI, romaneio e expedicao | pendente |
| `DEC-009` | Parcelas e posicao financeira/fiscal legada | criar parcelas normalizadas e eventos/snapshots de abertura para status recebido, comissao paga e referencia de NF incompleta, sem fabricar datas ou recebimentos | pedidos, faturamento, financeiro e comissoes | pendente |
| `DEC-010` | Campanhas, pontos e premiacao | criar campanhas e regras versionadas, ledger append-only de pontos/premios e vouchers; nao misturar premiacao com comissao | comercial, metas, financeiro e auditoria | pendente |
| `DEC-011` | Papeis do vinculo cliente-vendedor | distinguir no relacionamento quem cadastrou, quem atende e demais papeis, com vigencia e auditoria | cadastros, visibilidade comercial e pedidos | pendente |
