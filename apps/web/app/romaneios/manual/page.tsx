...
export default function ManualRomaneioPage() {
  const steps = [
    ...
    ["Grave o Romaneio", "A consulta de produtos, pesos e volumes não grava. Use Gravar rascunho do Romaneio quando a seleção estiver correta."],
    ...
    ["Informe entrega e NF", "Informe entregador, veículo e a referência da nota fiscal externa. A confirmação do Romaneio consolida a baixa física do estoque."],
  ...
  return (
    <main className="workspace dashboard-workspace manual-page">
      <div className="dashboard-header"><div><span className="eyebrow">manual operacional</span><h1>Como fazer um Romaneio</h1><p className="muted">Fluxo do pedido com saldo até a confirmação física da expedição.</p></div><Link className="primary-button" href="/romaneios">Voltar ao Romaneio</Link></div>
      ...
      <section className="notice-panel warning"><strong>Regra principal</strong><span>Pedido aberto não baixa estoque. Reserva reduz o saldo disponível. Registrar a referência fiscal não movimenta estoque. A baixa física ocorre somente ao confirmar o Romaneio.</span></section>
    </main>
  );
}
