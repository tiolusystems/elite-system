...
|---|---|---|---|---|---|
| seguranca | usuario autenticado ve proprio perfil, catalogo de permissoes e proprias excecoes | usuarios/permissoes somente por RPC administrativa | sim | `system.admin`, `security.manage_users`, `security.manage_permissions` | escrita direta em `user_profiles`, `permission_actions` e `user_permission_overrides` restringida na `0035`; perfis e overrides por RPC auditada; Supabase Auth continua responsavel por senha/login |
