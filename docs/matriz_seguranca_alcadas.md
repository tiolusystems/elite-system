...
|---|---|---|---|---|---|
| seguranca | usuario autenticado ve proprio perfil, catalogo de permissoes e proprias excecoes | usuarios/permissoes somente por RPC administrativa; criacao Auth por boundary server-only | sim | `system.admin`, `security.manage_users`, `security.manage_permissions`, `security.change_own_password` | escrita direta em `user_profiles`, `permission_actions` e `user_permission_overrides` restringida na `0035`; perfis e overrides por RPC auditada; 0038 autoriza criacao Auth por senha temporaria e forca troca no primeiro acesso; 0046 adiciona recuperacao por link e troca voluntaria sem registrar credencial |
