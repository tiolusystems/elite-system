...
function FinishForm({ order }: { order: PackagingOrder }) {
  return <form className="packaging-finish-form" action={finishPackagingAction}><input type="hidden" name="ordem_envase_id" value={order.id} /><div className="panel-header"><h4>Gerar lotes PA</h4><span className="pill">total obrigatório: {number(order.plannedFinishedPackages)}</span></div><div className="form-grid">{[1, 2, 3].map((index) => <div className="packaging-output-row" key={index}><label>Quantidade do lote {index}<input name={`lote_${index}_quantidade`} inputMode="decimal" /></label><label>Identificação/observação<input name={`lote_${index}_observacao`} placeholder="Opcional" /></label></div>)}<label className="full-field">Observação final<input name="observacao" /></label></div><div className="form-footer"><span>A soma deve corresponder à quantidade planejada.</span><button className="primary-button" type="submit">Finalizar e gerar PA</button></div></form>;
}
