...
| `/pedidos/listas-precos` | A/B | Baixar modelo, analisar XLSX, revisar conciliacao, publicar versao e consultar historico | alçadas independentes de leitura, importacao e publicacao de listas | codigo inexistente, nome divergente, faixa sobreposta, unidade/fator invalido, workbook repetido e retry divergente sao classificados ou negados sem efeito parcial | 360 a 1920 | N/A | manual proprio | migration 0137, smoke SQL, contratos Python e E2E dirigido | comprovado |
| `/pedidos/listas-precos/modelo` | D | Gerar o modelo XLSX governado com catalogos canonicos | leitura de listas de preco | sem sessao ou alcada o arquivo nao e emitido; endpoint nao grava fatos | N/A | XLSX | manual de listas | contrato Python e E2E dirigido | comprovado |
| `/custos-precos` | A/B | Criar e revisar politicas, congelar cenarios, calcular memorias e decidir calculos | cinco alcadas independentes de Precificacao | fonte ausente, componente invalido, denominador nao positivo, autoaprovacao e retry divergente sao recusados sem efeito parcial | 360 a 1920 | N/A | manual proprio | migration 0138, smoke SQL e contratos Python; E2E local bloqueado por browser indisponivel | comprovado com ressalva |
| `/custos-precos/export/[calculoId]/xlsx` | D | Exportar o snapshot aprovado em formato XLSX | leitura governada de Precificacao | somente calculo APPROVED; sem escrita; hash divergente bloqueia a entrega | N/A | XLSX | manual proprio | contrato de exportacao PRC + smoke SQL; snapshot aprovado e SHA-256 persistidos | comprovado |
| `/custos-precos/export/[calculoId]/pdf` | D | Exportar o snapshot aprovado em formato PDF | leitura governada de Precificacao | somente calculo APPROVED; sem escrita; hash divergente bloqueia a entrega | N/A | PDF | manual proprio | contrato de exportacao PRC + smoke SQL; snapshot aprovado e SHA-256 persistidos | comprovado |
...
| `/pedidos/financeiro/comissoes/relatorio` | B/D | Posição de comissões a pagar | leitura; exportação independente | acesso e exportação sem alçada negados | 360 a 1920 | A4, Excel e CSV | contextual específico | contrato OPS-FIN | em validação |
| `/pedidos/financeiro/comissoes/relatorio/export` | D | Exportar a posição filtrada de comissões em formato tabular | leitura e exportação independentes | exportação sem ambas as alçadas é negada | N/A | Excel principal; CSV alternativo | manual do relatório | contrato OPS-FIN + UX-SYS-01 | em validação |
| `/pedidos/financeiro/comissoes/relatorio/csv` | D | Compatibilidade da antiga URL de exportação CSV | leitura e exportação são revalidadas no endpoint canônico | redireciona para `/export?formato=csv`; sem alçada o endpoint final nega | N/A | CSV | manual do relatório | contrato OPS-FIN + compatibilidade | em validação |
| `/pedidos/financeiro/manual` | B | Guia operacional consolidado do Financeiro, sem executar movimentos | qualquer conta com ao menos uma alçada financeira efetiva | rota somente leitura; sem alçada financeira o acesso é negado | 360 a 1920 | N/A | a própria rota | contrato OPS-FIN + ajuda contextual UX-SYS-01 | em validação |
...
| `/producao/ordens/[id]` | A | Detalhe, reservas e mudanças de estado da OP selecionada | alçadas atômicas de OP | ações ausentes em modo somente leitura | 360 a 1920 | N/A | contextual | contratos de Produção + build | comprovado |
...
| `/qualidade/rastreabilidade` | B | Genealogia, recall e conciliacao | alçadas de Qualidade/Relatorios | lote ou cliente sem escopo e negado; divergencia nao e ocultada | 360 a 1920 | Excel/CSV governado | contextual | E2E rastreabilidade | comprovado |
| `/qualidade/rastreabilidade/export` | D | Exportar consulta auditavel | alçada de exportacao | filtro invalido ou sem acesso e recusado | N/A | Excel principal; CSV alternativo | herda Rastreabilidade | contrato de exportacao + UX-SYS-01 | comprovado |
...
| `/pedidos/listas-precos` | analisar workbook e publicar a analise aprovada | operador comercial autorizado | `pricing.lists.import` e `pricing.lists.publish` independentes | modelo governado, codigos canonicos, conciliacao sem erro e versao ainda nao publicada | staging auditavel e publicacao atomica da versao imutavel | SHA e idempotency key repetidos retornam o mesmo fato; divergencia, concorrencia e qualquer erro bloqueiam publicacao integral | migration 0137, smoke SQL e E2E dirigido | comprovado |
| `/custos-precos` | `createPricingPolicyAction`, `reviewPricingPolicyAction`, `createPricingScenarioAction`, `calculatePricingScenarioAction`, `reviewPricingCalculationAction` | operador ou revisor de Precificacao autorizado | `precificacao.policy.manage`, `precificacao.policy.review`, `precificacao.scenario.manage`, `precificacao.calculate`, `precificacao.calculation.review` independentes | politica, apresentacao e 11 fontes validas; segregacao entre criacao e aprovacao | fatos imutaveis de politica, cenario, calculo e decisao, sem publicar lista nem alterar preco contratado | ausencia de fonte, autoaprovacao, denominador invalido e retry divergente recusados; leitura e escrita direta permanecem default-deny | migration 0138, smoke SQL e contratos Python | comprovado com ressalva |
...
`analyzePriceListWorkbookAction`, `publishPriceListAnalysisAction`,
