...
export default async function InputTypesPage({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  const selected = catalog.inputTypes.find((item) => item.id === requestedId) ?? null;
  ...
  return (
    <CatalogShell
      ...
      actions={
        isListing ? (
          <Link className="primary-button" href="/cadastros/tipos-insumo?modo=novo#novo-tipo">Novo tipo</Link>
        ) : (
    ...
    >
      ...
      {isListing ? (
      <section className="catalog-list-view" aria-label="Consulta de tipos de insumo">
      ...
      <section>
        <article className="panel catalog-list-panel">
          ...
          <div className="catalog-record-list">
            {filtered.map((item) => <Link key={item.id} href={{pathname:"/cadastros/tipos-insumo",query:{q:singleParam(params.q)??"",status,selected:item.id}}}>
      ...
      {isCreating ? (
      <section className="panel form-panel catalog-create-view" id="novo-tipo" aria-labelledby="new-input-type-title" style={{ scrollMarginTop: 76 }}>
...
function buildListHref(query: string | null, status: string): string {
  ...
  return suffix ? `/cadastros/tipos-insumo?${suffix}` : "/cadastros/tipos-insumo";
}
