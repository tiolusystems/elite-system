  ...
  return (
    <main className="orders-workspace">
      ...
      <section className="panel orders-approvals" id="aprovacoes">
        ...
        {workspace.approvals.length ? <div className="approval-list">{workspace.approvals.map((order) => (
          <article key={order.id}>
            ...
            <p className="table-subtext">A decisao deste pedido nao altera o limite cadastral do cliente.</p>
      ...
      <section className="panel" id="historico"><div className="panel-header"><div><h2>{selected ? `Histórico de ${selected.clientName}` : "Pedidos recentes no seu escopo"}</h2><p>{selected ? "Somente pedidos do cliente selecionado." : "Vendedor: carteira própria. Gerente: carteira própria e equipe."}</p></div><span className="pill">{visibleOrders.length} pedido(s)</span></div>
        {visibleOrders.length ? <div className="orders-history"><div className="orders-history-head"><span>Pedido</span><span>Cliente</span><span>Vendedor</span><span>Situação</span><span>Total</span><span>Documento</span></div>{visibleOrders.map((order) => <article key={order.id}><strong>{order.code}</strong><span>{order.clientName}<small>{order.propertyName ?? "Sem propriedade específica"}</small></span><span>{order.sellerName ?? "Não informado"}</span><span className="status-chip">{statusLabel(order.status)}</span><strong>{money(order.total)}</strong>{["open", "fulfilled"].includes(order.status) ? <Link className="secondary-button" href={`/pedidos/${order.id}/contrato`} target="_blank">Exportar PDF</Link> : <span className="orders-document-pending">Disponível após aprovação</span>}</article>)}</div> : <div className="empty-state"><strong>{selected ? "Este cliente ainda não possui pedidos visíveis" : "Nenhum pedido no seu escopo"}</strong><span>Pedidos de outras carteiras não são exibidos.</span></div>}
      </section>
    </main>
  );
}
